...
for a 6-DOF, torque-actuated Franka FR3 in MuJoCo, built from scratch: the
end-effector dynamics (eq. 14-25), the unconstrained-motion control law
(eq. 28-31, 34), and the hybrid motion/force control law with generalized
task-specification matrices (eq. 1-4, 45-48).
...
-- see "Why the FR3 is reduced to 6 DOF" below for how that shaped the model.
...
- **6-DOF reduction over null-space damping**: see above.
